...
- [Проверка статов](https://github.com/DeadJacks/Check_Stats) - в предыстории наёмников отображается разброс статов и тип (боевой, низкорожденный). Добавляется черта которая пропадает на 11 уровне и показывает разброс статов к ветеранскому уровню. Требуется modern hooks.
 - [Эффективные очки здоровья]() - Рассчитывает и отображает эффективные хитпоинты персонажа (ЭфОЗ), которые представляют собой оценку общего количества ОЗ персонажа при снижении входящего урона. При расчете используются текущее здоровье персонажа, текущая прочность шлема и брони, показатели Steel Brow, Nimble, Battle Forged и активный Indomitable. Для удобства сравнения показаны текущий и «базовый» (без снижения урона) ЭфОЗ. Средние ЭфОЗ рассчитываются исходя из предположения, что 25% входящих атак попадают в голову, а 75% - в тело. Полученные значения ЭфОЗ являются теоретическими и в основном полезны для сравнения различных доспехов, братьев и билдов. Требуется mod hooks, modern hooks.
## 
